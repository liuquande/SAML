...
We frist convert the data from different sites uniformly to '.nii' format. For preprocessing, we first center-cropped the images from Site C with roughly same view as images from other sites, since the raw images of Site C are scanned from whole body rather than prostate surrounding area. We then resized all samples of six sites with size of $384 \! \times \! 384$ \revise{in axial plane. }
